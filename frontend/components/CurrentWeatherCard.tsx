...
function CurrentWeatherCard({ weather, unit, loading }: CurrentWeatherCardProps) {
  const icon = weather ? getWeatherIcon(weather.weather_icon) : "☀️";
  ...
  return (
    <div className={`card p-6 ${loading && !weather ? "animate-pulse" : ""}`}>
      {/* Loading skeleton */}
      {loading && !weather && (
