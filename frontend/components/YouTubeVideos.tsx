    ...
    <div className="card p-5">
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(2)].map((_, i) => <div key={i} className="h-16 bg-bg-hover rounded-xl animate-pulse" />)}
        </div>
      ) : (
        <div className="space-y-3">
          {videos.map((v) => (
            <a
              ...
              className="flex gap-3 items-center p-2 rounded-xl hover:bg-bg-hover transition-all group"
            >
              <img src={v.thumbnail} alt={v.title} className="w-20 h-12 rounded-lg object-cover flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-xs text-text-primary font-sans line-clamp-2 group-hover:text-accent-blue transition-colors">{v.title}</p>
